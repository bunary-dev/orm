...
- **Updated @bunary/core integration** - Upgraded from v0.0.2 to v0.0.7 and updated integration to work with instance-scoped config stores
  - `@bunary/core` is now a peer dependency (optional)
  - Updated `tryGetCoreConfig()` to work with new `createConfig()` API
  - Integration gracefully handles when `@bunary/core` is not installed
  - Added tests for core integration
